...
function JobDetails() {
  ...
  if (!job) return <div className="p-4 sm:p-8">Job not found.</div>;
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 sm:py-8">
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {job.tags.map(tag => (
          <span key={tag} className="px-2 py-1 bg-indigo-100 text-indigo-800 text-xs sm:text-sm rounded-full">{tag}</span>
        ))}
        <span className={`px-2 py-1 ${job.status === "archived" ? "bg-gray-300 text-gray-700" : "bg-green-100 text-green-800"} text-xs sm:text-sm rounded-full`}>
          {job.status}
        </span>
      </div>
      <Link to="/jobs" className="text-indigo-600 hover:underline text-sm sm:text-base">Back to Jobs</Link>
    </div>
  );
}
